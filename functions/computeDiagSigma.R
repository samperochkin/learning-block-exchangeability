computeDiagSigma <- function(X){
  
  l.ij.mat <- t(combn(1:d,2))
  tau.hat <- cor.fk(X)[l.ij.mat]
  ...
  sapply(1:p, function(r){
    tau.mod <- 2*(2*n - 3) / (n*(n - 1)) * (tau.hat[r] + 1)^2 
    (4 / (n * (n - 1)))^2 * (crossprod(IJ1.vec[,r],IJ2.vec[,r]) - crossprod(I2.mat[,r],IJ.mat[,r])) - tau.mod
  })
}