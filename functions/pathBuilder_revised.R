pathBuilder <- function(Tau.hat, Theta.hat, n, shrink){
  ...
  Sigma.hat <- buildSigma(Theta.hat, tau.hat, n, F)
  Sigma.list[[1]] <- Sigma.hat
  
  if(shrink == 0){
    Sigma2.list[[1]] <- Sigma.hat 
  }
  ...
  if(shrink == 1){
    shrinkS <- function(S){
      w <- 0
      S2 <- S
      # print(sum(abs(S2 - t(S2))))
      while(!is.positive.definite(S2)){
        w <- w + .01
        S2 <- w * diag(diag(S)) + (1 - w) * S
      }
      S2
      #(1 + w)/2 * diag(diag(S)) + (1 - w)/2 * S
    }
    Sigma2.list[[1]] <- shrinkS(Sigma.hat)
  }
  
  if(shrink == 2){
    Sigma2.list[[1]] <- diag(diag(Sigma.hat)) 
  }
  ...
  for(i in 2:(d-1)){
    ...
    Sigma.list[[i]] <- buildSigma(Theta.tilde, tau.tilde, n, F)
    if(sum(abs(Theta.tilde - t(Theta.tilde))) > .000001) print("WATCH")
    t2 <- Sys.time()
    # print(difftime(t2,t1))
    
    if(shrink == 0){
      Sigma2.list[[i]] <- Sigma.list[[i]]
    }
    
    if(shrink == 1){
      Sigma2.list[[i]] <- shrinkS(Sigma.list[[i]])
    }
    
    if(shrink == 2){
      Sigma2.list[[i]] <- diag(diag(Sigma.list[[i]])) 
    }
  ...
  tau.tilde <- ((Delta2 %*% (Tau.hat - diag(d)) %*% Delta2) / (Delta2 %*% (1 - diag(d)) %*% Delta2))[l.ij.mat]
  Theta.tilde <- computeTt(new.Delta, Theta.hat, m.lr.mat, lr.m.mat, m.shared.mat, l.ij.mat)
  Sigma.list[[d]] <- buildSigma(Theta.tilde, tau.tilde, n, F)
  
  if(shrink == 0){
    Sigma2.list[[d]] <- Sigma.list[[d-1]]
  }
  
  if(shrink == 1){
    Sigma2.list[[d]] <- shrinkS(Sigma.list[[d-1]])
  }
  
  if(shrink == 2){
    Sigma2.list[[d]] <- diag(diag(Sigma.list[[d-1]])) 
  }
