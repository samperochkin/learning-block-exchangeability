pathBuilder <- function(Tau.hat, Theta.hat, n, buffer = NULL, cutoff = 20, w = .1){
  ...
  bigWinner <- function(Delta){
    ...
    the_gal <- which.min(sapply(1:L, function(l){
      ...
      ind <- do.call(what = rbind, lapply(which(Delta[,pair[1]] == 1), function(i){
        cbind(rep(i,d-1),(1:d)[-i])
      }))
      
      clus <- Delta %*% (1:ncol(Delta))
      clus <- matrix(clus[c(ind)], ncol = 2)
      
      tau.tilde[ij.l.mat[ind]] <- Tt[clus]
      #t2 <- Sys.time()
  ...
  for(i in 2:(d-1)){
    ...
    #print("comp")
    #t1 <- Sys.time()
    winning.pair <- bigWinner(Delta.list[[i-1]])
    Delta[,winning.pair[1]] <- rowSums(Delta[,winning.pair])
    Delta <- Delta[,-winning.pair[2]]
    ...
    #tau.tilde <- ((Delta2 %*% (Tau.hat - diag(d)) %*% Delta2) / (Delta2 %*% (1 - diag(d)) %*% Delta2))[l.ij.mat]
    clus <- Delta %*% (1:ncol(Delta))
    clus <- matrix(clus[c(l.ij.mat)], ncol = 2)
    tau.tilde[ij.l.mat] <- Tt[clus]
    #t2 <- Sys.time()
    #print(difftime(t2,t1))
  ...
  #Delta2 <- matrix(1,d,d)
  #tau.tilde <- ((Delta2 %*% (Tau.hat - diag(d)) %*% Delta2) / (Delta2 %*% (1 - diag(d)) %*% Delta2))[l.ij.mat]
  #tau.tilde <- (Delta %*% Tt %*% t(Delta))[l.ij.mat]
  clus <- Delta %*% (1:ncol(Delta))
  clus <- matrix(clus[c(l.ij.mat)], ncol = 2)
  tau.tilde[ij.l.mat] <- Tt[clus]
